...
int main(int argc, char ** argv){
    ...
    if (T == 0) {
        ...
        std::vector<float> Cfvct(dim*dim);
        std::vector<float> b_transpose = MatrixTranspose(Bf.getMdata(), Bf.nrows(), Bf.ncols());

        matrixMult_Avx(Af.getMdata(), b_transpose, Cfvct, Af.nrows(), Af.ncols(), Bf.ncols(), time);
        ...
        std::vector<double> Cvct(dim * dim);
        std::vector<double> b_transpose = MatrixTranspose(B.getMdata(), B.nrows(), B.ncols());


        matrixMult_Avx(A.getMdata(), b_transpose, Cvct, A.nrows(), A.ncols(), B.ncols(), time);
    }
